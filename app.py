...
st.set_page_config(
    page_title="こいみら CSV変換",
    page_icon="🍒",
    layout="centered"
)

st.title("🍒 こいみら CSV→Excel変換")

# --- ファイル名カスタマイズ機能 ---
custom_filename = st.text_input(
    "保存するファイル名を変更したい場合は入力してください（空欄ならデフォルト）",
    placeholder="例：5月度イベント_オーダーまとめ"
)
# ---------------------------------

st.write("CSVファイルを選ぶと、フォーマットを整えたオーダーまとめExcelファイルに変換できます。")
st.caption("※ファイルは外部に送信されず、このパソコン上で処理されます。")
...
if uploaded_file is not None:
    try:
        ...
        if missing:
            st.error(f"次の必須列が不足しています: {missing}")
        else:
            st.success("CSVを正常に読み込みました！フォーマット変換の準備ができました。")
            ...
            # --- 💡 ここでファイル名の条件分岐を行っています ---
            if custom_filename.strip():
                # 入力がある場合（拡張子がなければ自動で付与）
        ...
        st.error("変換処理中にエラーが発生しました。CSVの形式を確認してください。")
        st.exception(e)